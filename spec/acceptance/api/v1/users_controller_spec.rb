...
resource "V1" do
  ...
  post "api/users/login" do
    ...
    # parameter :password_confirmation, "User's confirmation password", required: true, "Type" => "String", scope: :criteria
    ...
    example "Login: Returns Invalid email or password" do
      do_request
      expect(JSON.parse(response_body)["message"]).to eq("Invalid email or password")
